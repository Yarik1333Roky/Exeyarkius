...
async def queue_module(ctx, after:False):
    ...
    if can_playing==False:
        ...
        if datas > 0:
            ...
            voice.play(discord.FFmpegPCMAudio(executable="/Exeyarkius/blob/main/ffmpeg/ffmpeg.exe", source=url, **FFMPEG_OPTIONS)) #executable="/Exeyarkius/blob/main/ffmpeg/ffmpeg.exe", source
            can_playing=True
